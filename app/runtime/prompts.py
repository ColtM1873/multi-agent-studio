...
history_to_summery = 5

summery_prompt_prefix =  "<primary_objective>\nExtract the highest quality/most relevant context from the entire conversation history."

def summery_prompt_generator (usage_count : int , proactive_flush : bool):
   count_for_history = None
   count_for_summery = None
   numerical_unit_for_history = None
   numerical_unit_for_summery = None
   prompt1 = None
   prompt2 = None
...
"""
   ...
   if usage_count >= 10000:
      ...
      if usage_count // history_to_summery >= 10000:
         count_for_summery = usage_count // ( 10000 * history_to_summery)
         numerical_unit_for_summery = "万"
      elif usage_count // history_to_summery >= 1000:
         count_for_summery = usage_count // ( 1000 * history_to_summery)
         numerical_unit_for_summery = "千"
   elif usage_count >= 1000:
      count_for_history = usage_count // 1000
      numerical_unit_for_history = "千"
      if usage_count // history_to_summery >= 1000:
         count_for_summery = usage_count // ( 1000 * history_to_summery)
         numerical_unit_for_summery = "千"
   if not count_for_summery or not count_for_history:
      return summery_prompt

   length_prompt = f"""
鉴于整个会话长度此时已经达到了{count_for_history}{numerical_unit_for_history}tokens，因此请在总结时不要忌惮长度太长。输出大篇幅的总结是被鼓励的，否则不足以覆盖如此长的会话历史。具体来说，{count_for_summery}{numerical_unit_for_summery}tokens长度的总结都是可以接受的。
另外##注意##，你在本次回答中不应当调用任何的工具。
"""
   return summery_prompt + length_prompt


trimmed_summery_prompt = """
