...
基于 **LangGraph 的 Supervisor-Worker 多智能体框架**，配一套现代 Web 图形界面。
...
- **Streaming chat** — real-time Markdown rendering of agent / sub-agent output over WebSocket; sub-agent history aggregated by sub-agent name.
...
- Python 3.13+
- PostgreSQL with `pgvector` extension (for the checkpoint saver and semantic memory store)
...
Then in the browser:
1. Click **＋ 新建 multi-agent** and fill in the form (API key, system prompts, sub-agents, MCP servers, PostgreSQL databases).
